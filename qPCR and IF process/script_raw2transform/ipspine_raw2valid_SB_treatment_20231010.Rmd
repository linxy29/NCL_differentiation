---
title: Cleaning of ipspine raw data with SB treatment of october 10 2023
...
input_data_name <- "ipspine_data_SB_20231010.xlsx"
...
dt_tidy_l <- lapply(names_sheets_clean, function(k){
   print(paste("Sheet", k))
...
```{r working-check, eval = F,include = F}
error_control_ct
colnames(dt_tidy)

dt_tidy[exp == "20_31" & clone == "MIPS203.B11" & day == 7 & NOTO_mRNA == T & sb == T & sb_day == 2 & sb_conc == 5 & ct_control_gene == "GAPDH_SYBR"]

dt_tidy[exp == "20_31" & clone == "MIPS203.B11" & day == 7 & NOTO_mRNA == T & sb == T & sb_day == 2 & sb_conc == 5, .N, gene]

dt_tidy[,.N, sb_day]

dt_tidy[sb_day == 1, .N, Echantillon]
dt_tidy[sb_day == 5, .N, Echantillon]

dt_tidy[, .N, gene]

dt_tidy[gene %in% c("NOTO", "NOTO_SYBR", "NOTO_SYBR_SYBR"),.N]

dt_tidy[gene %in% c("NOTO", "NOTO_SYBR", "NOTO_SYBR_SYBR"),.N, .(exp, clone)]

dt_tidy[gene %in% c("NOTO_SYBR_SYBR"),.N]

dt_tidy[gene %in% c("NOTO_SYBR_SYBR"),.N, .(exp, clone)]
...
rules <- validate::validator(
   ...
   "sb_day_levels" = is.na(sb_day) | sb_day %vin% c(1,2,3,5),
   "sb_conc_levels" = is.na(sb_conc) | sb_conc %vin% c(5,10),
    "sb_false" = sb %vin% c(FALSE, TRUE), 
